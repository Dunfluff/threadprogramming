...
public class GUIMutex implements ActionListener {
	...
	/*
	 * Starts threads at run button press and clears lables at clear button
	 * press.
	 */
	@Override
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btnRun) {
			if (txtTrans.getText() != "") {
				System.out.println(txtTrans.getText());
				btnRun.setEnabled(false);
				int size = txtTrans.getText().length();
				new Writer(cb, lblTrans, listW, txtTrans.getText(), bSync.isSelected()).start();
				Reader red = new Reader(cb, lblRec, listR, bSync.isSelected(), size);
				red.start();
				new Checker(lblStatus, pnlRes, red, btnClear, btnRun, lblTrans, lblRec).start();
			}
		}
